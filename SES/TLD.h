...
public:
  ...
  void processFrame(const cv::Mat& img1,const cv::Mat& img2,std::vector<cv::Point2f>& points1,std::vector<cv::Point2f>& points2,
      BoundingBox& bbnext,bool& lastboxfound, bool tl,bool de, bool le, FILE* bb_file);
  ...
  void bbPredict_rot(const std::vector<cv::Point2f>& points1,const std::vector<cv::Point2f>& points2,
	  const BoundingBox& bb1,BoundingBox& bb2);
  ...
  BoundingBox pbox;
  int frames;
  void        InitiateTrack(IplImage *im, CvPoint2D32f  TargetPos);
  int        TrackOneImage(IplImage *im, CvPoint2D32f *TargetPos);
};
